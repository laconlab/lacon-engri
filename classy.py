...
class Word:
    ...
    classla_nonstd_lemmatizer = classla.Pipeline("hr", type="nonstandard")
    classla_std_lemmatizer = classla.Pipeline("hr")
    def __init__(self, word: str, freq: int) -> None:
        self.word = word
        self.freq = freq
        self.wn_lemma = self.wordnet_lemmatizer.lemmatize(word)
        self.nonstd_lemma = self.wordnet_lemmatizer.lemmatize(word)
        self.std_lemma = self.wordnet_lemmatizer.lemmatize(word)
        self.keep = False
...
def remove_words_without_aeiouy(words: List[Word]) -> List[Word]:
    return [word for word in words if len(AEIOUY - set(word.std_lemma)) != 6]

@show_progress
def remove_words_with_prefix(words: List[Word], prefix:List[str]) -> List[Word]:
    return [word for word in words if not any(word.std_lemma.startswith(p) for p in prefix)]

@show_progress
def remove_words_with_suffix(words: List[Word], suffix:List[str]) -> List[Word]:
    return [word for word in words if not any(word.std_lemma.endswith(s) for s in suffix)]

@show_progress
def remove_words_with_double_vowel_prefix(words: List[Word]) -> List[Word]:
    return [word for word in words if not starts_with_double_vowel(word.std_lemma)]
...
def remove_words_in_hml(words: List[Word]) -> List[Word]:
    ...
    for word in words:
        ...
        hml_wn_found = cache.get(word.std_lemma)
        hml_std_found = cache.get(word.std_lemma)
        hml_nonstd_found = cache.get(word.nonstd_lemma)
        ...
        elif hml_std_found is None: to_check_hml_words.append(word.std_lemma)
        elif hml_std_found: continue
        elif hml_nonstd_found is None: to_check_hml_words.append(word.nonstd_lemma)
        elif hml_nonstd_found: continue
...
def remove_using_manual_dataset(words: List[Word]) -> List[Word]:
    ...
    def any_in_cache(word: Word, tpe: int):
        return any(cache.get(w, -1) == tpe for w in [word.word, word.wn_lemma, word.std_lemma, word.nonstd_lemma])
    ret = []
    for word in words:
...
def predict(word: Word) -> Optional[Word]:
    ...
    elif model.predict([word.std_lemma]): return word
    elif model.predict([word.nonstd_lemma]): return word
    return None
